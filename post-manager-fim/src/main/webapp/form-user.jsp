			...
			<form action="${pageContext.request.contextPath}/user/${action}" method="POST">
				...
				<div class="row">
					...
					<div class="form-group col-md-3">
						...
						<select id="cargo" class="form-control selectpicker" name="cargo" 
							    required oninvalid="this.setCustomValidity('Por favor, informe o cargo do usuário.')"
							    oninput="setCustomValidity('')">
						  <option value="" disabled ${not empty company ? "" : "selected"}>Selecione um usuário</option>
						  <c:forEach var="cargo" items="${cargos}">
						  	<option value="${cargo.getIdCargo()}"  ${cargo.getIdCargo() == user.getCargo().getIdCargo() ? "selected" : ""}>
						  		${cargo.getNomeCargo()}
						  	</option>	
						  </c:forEach>
						</select>
