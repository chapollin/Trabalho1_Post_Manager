...
public class MySQLUserDAO implements UserDAO {
	...
	public boolean save(User user) throws ModelException {
		...
		db.setInt(4, user.getCargo().getIdCargo());
	...
	public boolean update(User user) throws ModelException {
		...
		String sqlUpdate = "UPDATE users SET nome=?, sexo=?, email=?, cargo=? WHERE id=?";
	...
	private User createUser(DBHandler db) throws ModelException {
		...
		CargoDAO cargoDAO = DAOFactory.createDAO(CargoDAO.class);
		Cargo cargo = cargoDAO.findById(db.getInt("cargo"));
